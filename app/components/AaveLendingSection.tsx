...
interface AaveLendingSectionProps {
    ...
    <div className="px-1 mb-8">
      <div className="flex items-center">
        <div className="flex-auto">
          <h1 className="text-base font-semibold">Aave Lending</h1>
        </div>
      </div>
              ...
              <tbody className={styles.background}>
                {aavePositionsWithTokens.map((item) => (
                  <tr key={item.token.symbol} className={`${styles.highlightRow} hover:cursor-pointer`} onClick={handleRowClick}>
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 truncate">{`${item.depositAPY}%`}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export default AaveLendingSection;
