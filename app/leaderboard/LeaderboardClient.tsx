...
export default function LeaderboardClient() {
  ...
  const sortedPortfolios = [...portfolios].sort((a, b) => parseFloat(b.totalUSDValue) - parseFloat(a.totalUSDValue))
  ...
  const getHiroBalance = (portfolio: Portfolio) => {
    const hiroToken = portfolio.tokens.find((token) => token.symbol.toUpperCase() === 'HIRO')
      ...
      <div className="bg-gray-800 rounded-lg overflow-hidden">
        ...
        {sortedPortfolios.map((portfolio, index) => (
          <div key={portfolio.address} className="border-t border-gray-700">
            <div className="grid grid-cols-4 p-4 cursor-pointer hover:bg-gray-750 items-center" onClick={() => toggleExpand(index)}>
